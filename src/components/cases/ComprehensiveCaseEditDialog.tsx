...
import { VictimEditSection } from "./edit-sections/VictimEditSection";
import { EvidenceEditSection } from "./edit-sections/EvidenceEditSection";
import { ReportEditSection } from "./edit-sections/ReportEditSection";

interface ComprehensiveCaseEditDialogProps {
  case: Case | null;
  open: boolean;
...
export function ComprehensiveCaseEditDialog({ case: caseData, open, onOpenChange, onCaseUpdated }: ComprehensiveCaseEditDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle>Edit Case - {caseData.caseNumber}</DialogTitle>
          <DialogDescription>
            Update case information, manage victims, evidence, suspects, and reports
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto mt-4 pr-2">
            ...
            <TabsContent value="suspects" className="space-y-4 m-0">
              <div className="space-y-6">
                <SuspectForm 
                  caseId={caseData.id} 
                  onSuspectAdded={() => {
                    // No need to reload the entire page, the SuspectsList will refresh
                  }}
                />
                <SuspectsList caseId={caseData.id} />
              </div>
            </TabsContent>

            <TabsContent value="victims" className="space-y-4 m-0">
              <VictimEditSection caseId={caseData.id} />
            </TabsContent>

            <TabsContent value="evidence" className="space-y-4 m-0">
              <EvidenceEditSection caseId={caseData.id} />
            </TabsContent>

            <TabsContent value="reports" className="space-y-4 m-0">
              <ReportEditSection caseId={caseData.id} />
            </TabsContent>
          </div>
